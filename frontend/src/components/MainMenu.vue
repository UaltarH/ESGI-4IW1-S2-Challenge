<template>
  <header class="bg-primary w-full h-12 fixed z-10">
    <nav class="lg:flex w-full lg:w-xl h-12 lg:items-center lg:flex-row m-auto relative">
      <h1 class="lg:flex-1 h-full">
        <RouterLink to="/" class="flex justify-center items-center lg:block h-full">
          <logo-with-text></logo-with-text>
        </RouterLink>
      </h1>
      <button type="button" class="absolute top-0 right-0 z-20 lg:hidden" id="burger" @click="toggleMenu">
        ...
        <ul class="flex flex-col lg:flex-row w-full lg:flex-2 py-32 lg:py-0 text-center font-bold">
          <li v-for= "(item, index) in menuItems" class="menu-item static lg:relative">
            ...
            <ul v-if="item.hasOwnProperty('children')" class="menu-submenu static lg:absolute">
              <li v-for="(subItem, indexSub) in item.children" class="menu-item menu-subitem h-12 text-center">
                <RouterLink :to="subItem.route" class="font-medium">{{ subItem.title }}</RouterLink>
              </li>
            </ul>
